    ...
    <>
      ...
      .ue-skel {
        background: linear-gradient(
          90deg,
          rgba(182,138,204,.07) 25%,
          rgba(182,138,204,.16) 50%,
          rgba(182,138,204,.07) 75%
        );
        background-size: 700px 100%;
        animation: ue-shimmer 1.6s infinite linear;
        border-radius: 6px;
        display: block;
      }
    `}</style>
      <div className='hero-glow gridBgDark noise bg-[#08040F]'>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-16 pt-16 pb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-12 mb-14">
            ...
            {/* Brand col */}
            <div>
              <span className="ue-skel" style={{ width: 160, height: 44, borderRadius: 8, marginBottom: 24 }} />
              <div className="flex flex-col gap-2 mb-7">
                {[100, 92, 97, 85, 60].map((w, i) => (
                  <span key={i} className="ue-skel" style={{ width: `${w}%`, height: 11 }} />
                ))}
              </div>
            </div>
            ...
            {/* Nav col 1 */}
            <div>
              <span className="ue-skel" style={{ width: 55, height: 9, marginBottom: 20 }} />
              <div className="flex flex-col gap-3.5">
                {[70, 55, 80, 60, 45].map((w, i) => (
                  <span key={i} className="ue-skel" style={{ width: `${w}%`, height: 12 }} />
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2.5 mb-7">
                {[80, 55, 90].map((w, i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <span className="ue-skel flex-shrink-0" style={{ width: 14, height: 14, borderRadius: '50%', marginTop: 1 }} />
                    <span className="ue-skel" style={{ width: `${w}%`, height: 10 }} />
                  </div>
                ))}
              </div>
              <div className="flex gap-2">
                {[0, 1, 2, 3].map((i) => (
                  <span key={i} className="ue-skel" style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0 }} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-7 mb-12 flex flex-col md:flex-row items-center justify-between gap-6"
            ...
            <div>
              <span className="ue-skel" style={{ width: 260, height: 14, marginBottom: 10 }} />
              <span className="ue-skel" style={{ width: 200, height: 11 }} />
            </div>
            <div className="flex gap-2 w-full md:w-auto">
              <span className="ue-skel" style={{ width: 240, height: 40, borderRadius: 12 }} />
              <span className="ue-skel" style={{ width: 100, height: 40, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg,rgba(196,114,34,.22),rgba(249,162,79,.15))' }} />
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-5 pt-8"
            ...
            <div className="flex items-center gap-2">
              <span className="ue-skel flex-shrink-0" style={{ width: 14, height: 14, borderRadius: '50%' }} />
              <span className="ue-skel" style={{ width: 280, height: 11 }} />
            </div>
            <div className="flex items-center gap-5">
              {[64, 56, 72].map((w, i) => (
                <span key={i} className="flex items-center gap-5">
                  <span className="ue-skel" style={{ width: w, height: 11 }} />
                  {i < 2 && <span style={{ color: '#2A1040' }}>·</span>}
                </span>
              ))}
            </div>
          </div>
